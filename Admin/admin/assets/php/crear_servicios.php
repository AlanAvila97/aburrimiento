        ...
        if (in_array(strtolower($extension), $formatosPermitidos)) {
            ...
            if (move_uploaded_file($imagenTmp, $rutaImagenOriginal)) {
                ...
                if ($imagen) {
                    ...
                    $sql = "INSERT INTO productos (titulo, description, tipo, imagen, imagenw) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $conexion->prepare($sql);
                    $stmt->bind_param("sssss", $titulo, $descripcion, $tipo, $rutaImagenOriginal, $imagenWebpNombre);
