        ...
        if ($tipoImagen === IMAGETYPE_JPEG || $tipoImagen === IMAGETYPE_PNG) {
            ...
            if (move_uploaded_file($imagenTmp, $rutaImagenOriginal)) {
                ...
                if ($imagen) {
                    ...
                    $sql = "UPDATE servicios_postventa SET p1 = ?, p2 = ?, p3 = ?, p4 = ?, titulo = ?, subtitulo = ?, descripcion = ?, imagenN = ?, imagenW = ? WHERE id = ?";
                    $stmt = $conexion->prepare($sql);
                    $stmt->bind_param("sssssssssi", $p1,$p2,$p3,$p4,$titulo, $subtitulo, $sinopsis, $imagenNombre, $imagenWEBP, $id);
